document.querySelector(".verify1").addEventListener("click", function() {
  ...
  //parseInt = transform a string into a number
    ...
    let achitareLuna = sumaTotalaCredit / nrLuni;
    document.querySelector('.result1').innerHTML = `Suma totala a dobanzii este <b>${sumaTotalaDobanda}</b> iar suma lunara a dobanzii este <b>${sumaLunaraDobanda}</b>. <br> Suma totala pentru achitarea creditului la final este <b>${sumaTotalaCredit}</b>. <p><p> Va trebui sa platiti lunar <b>${achitareLuna}</b>.</p></p>`;
  }
});